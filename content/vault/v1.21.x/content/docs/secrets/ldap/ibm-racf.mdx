...
- **Create a RACF-enabled LDAP user account for Vault**. You must have a 
  RACF-enabled LDAP user account with sufficient privileges to manage the 
  passwords/passphrases of the other RACF accounts that Vault will be 
  updating, including the RACF authority to update those passwords/passphrases 
  with the NOEXPIRED option.
